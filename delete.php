...
if ( isset( $_POST["submit"] ) ) {
	...
	if ( $maintenance_request->delete($ID) == false ) {
		$error = $pdo->errorInfo() ;
		$_SESSION["delete"] = 1;
		$_SESSION["delete_error"] = $error[2];
		header ( "Location:delete_seach_display.php" );
		exit();
	} else {
		$_SESSION["delete"] = 2;
		header ( "Location:delete_seach_display.php" );
...
<body>
	...
	<main>
		...
		<article class="jumbotron jumbotron-fluid text-center mb-0" style="width:100%;">
			...
			<h1>削除の確認</h1>
			<div class="result">
				<form id="form<?= $id ?>" name="form<?= $id ?>" action="" method="POST" onsubmit="return confirm('本当に削除してよろしいですか？')">
					<table class="input_area" style="margin-bottom:16px;" border="1" width="90%">
						<tr style="display:none;">
							<th>ID</th><th><input name="ID" value="<?= $id ?>"/></th>
						</tr>
						<tr>
							<td>入庫日</td><td><?= $nyuko_bi ?><input style="display:none;" name="nyuko_bi" value="<?= $nyuko_bi ?>"/></td>
						</tr>
						<tr>
							<td>返却予定日</td><td><?= $nousya_yoteibi ?><input style="display:none;" name="nousya_yoteibi" value="<?= $nousya_yoteibi ?>"/></td>
						</tr>
						<tr>
							<td>お客様氏名</td><td><?= $sei ?><input style="display:none;" name="sei" value="<?= $sei ?>"/><?= $mei ?><input style="display:none;" name="mei" value="<?= $mei ?>"/></td>
						</tr>
						<tr>
							<td>電話番号</td><td><?= $tel ?><input style="display:none;" name="tel" value="<?= $tel ?>"/></td>
						</tr>
						<tr>
							<td>メールアドレス</td><td><?= $mail ?><input style="display:none;" name="mail" value="<?= $mail ?>"/></td>
						</tr>
						<tr>
							<td>車種名</td><td><?= $car_name ?><input style="display:none;" name="car_name" value="<?= $car_name ?>"/></td>
						</tr>
						<tr>
							<td>型式</td><td><?= $katasiki ?><input style="display:none;" name="katasiki" value="<?= $katasiki ?>"/></td>
						</tr>
						<tr>
							<td>登録番号</td><td><?= $tourokubangou ?><input style="display:none;" name="tourokubangou" value="<?= $tourokubangou ?>"/></td>
						</tr>
						<tr>
							<td>車検証の有効期限</td><td><?= $syakenmanryou_bi ?><input style="display:none;" name="syakenmanryou_bi" value="<?= $syakenmanryou_bi ?>"/></td>
						</tr>
						<tr>
							<td>整備の種類</td><td><?= $seibi_syurui ?><input style="display:none;" name="seibi_syurui" value="<?= $seibi_syurui ?>"/></td>
						</tr>
						<tr>
							<td>整備内容</td><td><?= $seibi_naiyou ?><input style="display:none;" name="seibi_naiyou" value="<?= $seibi_naiyou ?>"/></td>
						</tr>
						<tr>
							<td>洗車の有無</td><td><?= $sensya ?><input style="display:none;" name="sensya" value="<?= $sensya ?>"/></td>
						</tr>
						<tr>
							<td>車内清掃の有無</td><td><?= $syanaiseisou ?><input style="display:none;" name="syanaiseisou" value="<?= $syanaiseisou ?>"/></td>
						</tr>
						<tr>
							<td>特記事項</td><td><?= $tokki_zikou ?><input style="display:none;" name="tokki_zikou" value="<?= $tokki_zikou ?>"/></td>
						</tr>
						<tr>
							<td>特記事項詳細</td><td><?= $tokki_zikou_syousai ?><input style="display:none;" name="tokki_zikou_syousai" value="<?= $tokki_zikou_syousai ?>"/></td>
						</tr>
					</table>
					<div class="btn_area">
						<input class="small_btn pl-5 pr-5" type="submit" name="submit" id="submit" value="削除" />
					</div>
				</form>
			</div>
			<!-- InstanceEndEditable -->
